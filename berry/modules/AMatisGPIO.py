...
class AMatisGPIO(ModuleBase):
	...
	def moduleExec(self,
		...
		for strCmd in ["ampel", "alarm", "status", "alive"]:
			# Wurde eines der unterstützten Kommandos angefordert?
			if not strCmd in dictQuery:
				continue
			# Die einzelnen Argumente des jeweiligen Kommandos abrufen
			for strArg in dictQuery[strCmd]:
				if (strCmd == "ampel"):
					ampel(strArg)
				elif (strCmd == "alarm"):
					alarm(strArg)
